...
public class BonecpConnPool {
	...
	static {
		...
		try {
			...
			prop.load(inStream);
			// 根据相应的键获得对应的值
			SQL_DRIVER = prop.getProperty("jdbc.driverClassName");
			SQL_URL = prop.getProperty("jdbc.url");
			SQL_USERNAME = prop.getProperty("jdbc.username");
			SQL_PASSWORD = prop.getProperty("jdbc.password");
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
